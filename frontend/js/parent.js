  ...
  try {
    ...
    materialsGrid.innerHTML = materials.map(material => `
      <div class="material-card">
        ...
        <a href="https://www.inf.unibz.it/~calvanese/teaching/04-05-ip/lecture-notes/uni01.pdf" target="_blank" class="download-btn">
          <i class="fas fa-download"></i> Download
        </a>
      </div>
    `).join('');
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.querySelectorAll('nav ul li a').forEach(link => {
    link.addEventListener('click', (e) => {
      ...
      // If not a hash link, let browser handle navigation (e.g., parent-payment.html)
    });
  });
